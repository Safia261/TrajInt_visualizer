...
def main():
    ...
    if args.input_mode == "single":
        ...
        if not args.no_smoothing_kalman and args.dataset.startswith("ctv"):
            ...
            t, a, c_da = compute_direction_angle_velocity_based(df, 3, 9, cfg["fps"], plot=True, return_class=True)
            print("\nDirection angle : ", c_da)
            t, a, c_aa = compute_approach_angle(df, 3, 9, cfg["fps"], plot=True, return_class=True)
            print("\nApproach angle : ", c_aa)
            times, rel_speeds, rel_speeds_kmh, angles, distances, c_v = compute_relative_speed(df, 3, 9, cfg["fps"], plot=True, return_class=True)
            print("\nRelative speed : ", c_v)
            pet = compute_pet(df, 3, 9, cfg["fps"], plot=True, return_class=True)
            print("\nPET : ", pet)
            times, ttc_val, ttc_min, c_ttc = compute_ttc(df, 3, 9, cfg["fps"], plot=True, return_class=True)    
            print("\n TTC : ", ttc_min, c_ttc)
            res = classify_global_interaction(pet, (ttc_min, c_ttc), c_aa, c_da, c_v)
            print("\nFinal results : ", res)
            i_fps, i_sec, duree_sec = compute_interaction_intervals(df, 3, 9, cfg["fps"])
